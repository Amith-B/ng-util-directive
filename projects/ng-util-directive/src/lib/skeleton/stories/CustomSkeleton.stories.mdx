...
1. Pass custom component as `component` prop for each `ngSkeleton` directive
...
1. Declare the abstract method `handleDataChange` which accepts the `data` as parameter, this method will be called with the new data whenever it is updated with `data` prop of `ngSkeleton`
